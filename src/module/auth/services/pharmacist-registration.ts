...
const pharmacistRegistration: AppRouteHandler<AuthRoutes["pharmacistRegistration"]> = async (ctx) => {
  ...
  const user = await prisma.user.create({
    data: { email, password: hashedPassword, firstName, lastName, phone, role: "PHARMACIST", createdBy: tokenData.id },
  });
